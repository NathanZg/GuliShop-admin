<template>
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column prop="nickName" label="用户昵称" align="center" width="300"/>

      <el-table-column prop="gmtCreate" label="创建时间" align="center" width="400"/>

      <el-table-column label="操作" width="400" align="center">
        <template slot-scope="scope">
          <router-link :to="'/acl/user/role/'+scope.row.id">
            <el-button type="info" size="mini" icon="el-icon-info" v-if="hasPerm('user.assgin')">设置角色</el-button>
          </router-link>
          <router-link :to="'/acl/user/update/'+scope.row.id">
            <el-button type="primary" size="mini" icon="el-icon-edit" v-if="hasPerm('user.update')">编辑用户</el-button>
          </router-link>
          <el-button type="danger" size="mini" icon="el-icon-delete" @click="removeDataById(scope.row.id)" v-if="hasPerm('user.remove')">删除用户</el-button>
        </template>
      </el-table-column>
    </el-table>
