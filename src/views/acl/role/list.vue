<template>
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column prop="roleName" label="角色名称" align="center" width="800"/>


      <el-table-column label="操作" width="420" align="center">
        <template slot-scope="scope">
          <router-link :to="'/acl/role/distribution/'+scope.row.id">
            <el-button type="info" size="mini" icon="el-icon-info" v-if="hasPerm('role.acl')">设置权限</el-button>
          </router-link>
          <router-link :to="'/acl/role/update/'+scope.row.id">
            <el-button type="primary" size="mini" icon="el-icon-edit"  v-if="hasPerm('role.update')">编辑角色</el-button>
          </router-link>
          <el-button type="danger" size="mini" icon="el-icon-delete" @click="removeDataById(scope.row.id)" v-if="hasPerm('role.remove')">删除角色</el-button>
        </template>
      </el-table-column>
    </el-table>
